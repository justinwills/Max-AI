...
(function () {
  ...
  function markLessonComplete(lessonId, itemOverride) {
    try {
      ...
      // Log a quiz attempt with timestamp for challenges/streaks on Home
      try {
  ...
  function updateProgress() {
    ...
    try {
      const tKey = K("neural_networks_quiz_total_v1");
      const dKey = K("neural_networks_quiz_done_v1");
    ...
    try {
      if (
        ...
        const cKey = K("neural_networks_completed_v1");
        ...
        try {
          const actKey = K("home_activity_logged_neural_networks_v1");
