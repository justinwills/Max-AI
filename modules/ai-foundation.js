...
(function () {
  ...
  codeBtn &&
    codeBtn.addEventListener("click", function () {
      const code = ``;
      download("ai-foundations-sample.js", code, "application/javascript");
    });
})();
  ...
  function specForLesson(item, lessonId, index) {
    ...
    if (title.includes("categories of ai")) {
      return {
        ...
        options: [
          ["a", "AI designed for specific tasks with limited scope", true],
          ["b", "AI that matches human general intelligence in all tasks"],
    ...
    if (title.includes("basic logic") || title.includes("search algorithm")) {
      return {
        type: "mcq",
    ...
    if (title.includes("overfitting") || title.includes("generalization")) {
      return {
        type: "mcq",
        prompt: "Overfitting best describes a model that:",
        options: [
          ["a", "Performs well on training but poorly on unseen data", true],
